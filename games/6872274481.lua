...
local tenacityEvents = setmetatable({}, {
...
if not tenacity.guiscale and tenacity.gui and tenacity.gui:FindFirstChild('ScaledGui') then
	tenacity.guiscale = tenacity.gui.ScaledGui:FindFirstChildOfClass('UIScale') or {Scale = 1}
end
...
local uipallet = tenacity.Libraries.uipallet
local tween = tenacity.Libraries.tween
local color = tenacity.Libraries.color
...
local getcustomasset = tenacity.Libraries.gettenacityasset or tenacity.Libraries.getcustomasset
...
local function hotbarSwitch(slot)
	if slot and store.inventory.hotbarSlot ~= slot then
		...
		tenacityEvents.InventoryChanged.Event:Wait()
		return true
	end
	return false
end
			...
			if hum and humrootpart then
				...
				if plr == lplr then
					...
					table.insert(entitylib.Connections, char.AttributeChanged:Connect(function(attr)
						tenacityEvents.AttributeChanged:Fire(attr)
					end))
				else
...
run(function()
	...
	local function updateStore(new, old)
		...
		if new.Inventory ~= old.Inventory then
			...
			if newinv ~= oldinv then
				tenacityEvents.InventoryChanged:Fire()
			end

			if newinv.inventory.items ~= oldinv.inventory.items then
				tenacityEvents.InventoryAmountChanged:Fire()
	...
	for _, event in {'MatchEndEvent', 'EntityDeathEvent', 'BedwarsBedBreak', 'BalloonPopped', 'AngelProgress', 'GrapplingHookFunctions'} do
		if not tenacity.Connections then return end
		bedwars.Client:WaitFor(event):andThen(function(connection)
			tenacity:Clean(connection:Connect(function(...)
				tenacityEvents[event]:Fire(...)
			end))
		end)
	end

	tenacity:Clean(bedwars.ZapNetworking.EntityDamageEventZap.On(function(...)
		tenacityEvents.EntityDamageEvent:Fire({
			entityInstance = ...,
			damage = select(2, ...),
			damageType = select(3, ...),
	...
	for _, event in {'PlaceBlockEvent', 'BreakBlockEvent'} do
		tenacity:Clean(bedwars.ZapNetworking[event..'Zap'].On(function(...)
			local data = {
				blockRef = {
					blockPosition = ...,
				},
			...
			tenacityEvents[event]:Fire(data)
		end))
	end
	...
	task.spawn(function()
		pcall(function()
			repeat task.wait() until store.matchState ~= 0 or tenacity.Loaded == nil
	...
	tenacity:Clean(tenacityEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
		if bedTable.player and bedTable.player.UserId == lplr.UserId then
			beds:Increment()
		end
	end))

	tenacity:Clean(tenacityEvents.MatchEndEvent.Event:Connect(function(winTable)
		if (bedwars.Store:getState().Game.myTeam or {}).id == winTable.winningTeamId or lplr.Neutral then
			wins:Increment()
		end
	end))

	tenacity:Clean(tenacityEvents.EntityDeathEvent.Event:Connect(function(deathTable)
	...
	pcall(function()
		if getthreadidentity and setthreadidentity then
			...
			task.spawn(function()
				repeat
					task.wait(0.1)
				until tenacity.Loaded == nil or bedwars.AppController:isAppOpen('BedwarsItemShopApp')
...
run(function()
	...
	AimAssist = tenacity:Module('Offense', {
		Name = 'AimAssist',
...
run(function()
	...
	AutoClicker = tenacity:Module('Offense', {
		Name = 'AutoClicker',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Sprint = tenacity:Module('Offense', {
		Name = 'Sprint',
		Function = function(callback)
			if callback then
				if inputService.TouchEnabled then 
...
run(function()
	...
	TriggerBot = tenacity:Module('Offense', {
		Name = 'TriggerBot',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Velocity = tenacity:Module('Offense', {
		Name = 'Velocity',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AntiFall = tenacity:Module('Motion', {
		Name = 'AntiFall',
		Function = function(callback)
			if callback then
							...
							if Mode.Value == 'Normal' then
								...
								if top then
									...
									connection = runService.PreSimulation:Connect(function()
										if tenacity.Modules.Fly.Enabled or tenacity.Modules.InfiniteFly.Enabled or tenacity.Modules.LongJump.Enabled then
...
run(function()
	...
	FastBreak = tenacity:Module('Motion', {
		Name = 'FastBreak',
		Function = function(callback)
			if callback then
				repeat
	...
	Fly = tenacity:Module('Motion', {
		...
		Function = function(callback)
			...
			if callback then
				...
				Fly:Clean(tenacityEvents.AttributeChanged.Event:Connect(function(changed)
...
run(function()
	...
	HitBoxes = tenacity:Module('Motion', {
		Name = 'HitBoxes',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Killaura = tenacity:Module('Motion', {
		Name = 'Killaura',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local LongJumpMethods = {
		...
		cat = function(_, _, dir)
			LongJump:Clean(tenacityEvents.CatPounce.Event:Connect(function()
	...
	LongJump = tenacity:Module('Motion', {
		Name = 'LongJump',
		Function = function(callback)
			frictionTable.LongJump = callback or nil
			updateVelocity()
			if callback then
				LongJump:Clean(tenacityEvents.EntityDamageEvent.Event:Connect(function(damageTable)
				...
				LongJump:Clean(tenacityEvents.GrapplingHookFunctions.Event:Connect(function(dataTable)
...
run(function()
	...
	local ProjectileAimbot = tenacity:Module('Motion', {
...
run(function()
	...
	ProjectileAura = tenacity:Module('Motion', {
		Name = 'ProjectileAura',
		Function = function(callback)
			if callback then
				repeat
	...
	Health = tenacity:Module('Vision', {
		...
		Function = function(callback)
			if callback then
				...
				Health:Clean(label)
				Health:Clean(tenacityEvents.AttributeChanged.Event:Connect(function()
					label.Text = entitylib.isAlive and math.round(lplr.Character:GetAttribute('Health'))..' ❤️' or ''
					label.TextColor3 = entitylib.isAlive and Color3.fromHSV((lplr.Character:GetAttribute('Health') / lplr.Character:GetAttribute('MaxHealth')) / 2.8, 0.86, 1) or Color3.new()
				end))
			end
...
run(function()
	...
	NameTags = tenacity:Module('Vision', {
		Name = 'NameTags',
		Function = function(callback)
			if callback then
...
run(function()
	...
	StorageESP = tenacity:Module('Vision', {
		Name = 'StorageESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoBalloon = tenacity:Module('Utility', {
		Name = 'AutoBalloon',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoKit = tenacity:Module('Utility', {
		Name = 'AutoKit',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoPearl = tenacity:Module('Utility', {
		Name = 'AutoPearl',
		Function = function(callback)
			if callback then
				local check
...
run(function()
	...
	AutoPlay = tenacity:Module('Utility', {
		Name = 'AutoPlay',
		Function = function(callback)
			if callback then
				AutoPlay:Clean(tenacityEvents.EntityDeathEvent.Event:Connect(function(deathTable)
					if deathTable.finalKill and deathTable.entityInstance == lplr.Character and isEveryoneDead() and store.matchState ~= 2 then
						joinQueue()
					end
				end))
				AutoPlay:Clean(tenacityEvents.MatchEndEvent.Event:Connect(joinQueue))
			end
		end,
...
run(function()
	...
	AutoToxic = tenacity:Module('Utility', {
		Name = 'AutoToxic',
		Function = function(callback)
			if callback then
				AutoToxic:Clean(tenacityEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
				...
				AutoToxic:Clean(tenacityEvents.EntityDeathEvent.Event:Connect(function(deathTable)
				...
				AutoToxic:Clean(tenacityEvents.MatchEndEvent.Event:Connect(function(winstuff)
					if GG.Enabled then
						if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
...
run(function()
	...
	AutoVoidDrop = tenacity:Module('Utility', {
		Name = 'AutoVoidDrop',
		Function = function(callback)
			if callback then
...
run(function()
	...
	MissileTP = tenacity:Module('Utility', {
		Name = 'MissileTP',
		Function = function(callback)
			if callback then
				MissileTP:Toggle()
...
run(function()
	...
	PickupRange = tenacity:Module('Utility', {
		Name = 'PickupRange',
		Function = function(callback)
			if callback then
...
run(function()
	...
	RavenTP = tenacity:Module('Utility', {
		Name = 'RavenTP',
		Function = function(callback)
			if callback then
				RavenTP:Toggle()
...
run(function()
	...
	Scaffold = tenacity:Module('Utility', {
		Name = 'Scaffold',
		Function = function(callback)
			if label then
...
run(function()
	...
	ShopTierBypass = tenacity:Module('Utility', {
		Name = 'ShopTierBypass',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local function playerAdded(plr)
		...
		elseif getRole(plr, 5774246) >= 100 then
			...
			if table.find(blacklistedclans, plr:GetAttribute('ClanTag')) and tenacity.Loaded and Clans.Enabled then
				connection:Disconnect()
	...
	StaffDetector = tenacity:Module('Utility', {
		Name = 'StaffDetector',
		Function = function(callback)
			if callback then
	...
	task.spawn(function()
		repeat task.wait(1) until tenacity.Loaded or tenacity.Loaded == nil
		if tenacity.Loaded and not StaffDetector.Enabled then
...
run(function()
	TrapDisabler = tenacity:Module('Utility', {
		Name = 'TrapDisabler',
...
run(function()
	...
	AutoSuffocate = tenacity:Module('World', {
		Name = 'AutoSuffocate',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	AutoTool = tenacity:Module('World', {
		Name = 'AutoTool',
		Function = function(callback)
			if callback then
...
run(function()
	...
	BedPlates = tenacity:Module('World', {
		Name = 'BedPlates',
		Function = function(callback)
			if callback then
				...
				BedPlates:Clean(tenacityEvents.PlaceBlockEvent.Event:Connect(refreshNear))
				BedPlates:Clean(tenacityEvents.BreakBlockEvent.Event:Connect(refreshNear))
...
run(function()
	...
	BedProtector = tenacity:Module('World', {
		Name = 'BedProtector',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ChestSteal = tenacity:Module('World', {
		Name = 'ChestSteal',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Schematica = tenacity:Module('World', {
		Name = 'Schematica',
		Function = function(callback)
			if callback then
	...
	AutoBuy = tenacity:Module('Player', {
		...
		Function = function(callback)
			if callback then
				...
				local lastupgrades
				AutoBuy:Clean(tenacityEvents.InventoryAmountChanged.Event:Connect(function()
...
run(function()
	...
	AutoConsume = tenacity:Module('Player', {
		...
		Function = function(callback)
			if callback then
				AutoConsume:Clean(tenacityEvents.InventoryAmountChanged.Event:Connect(consumeCheck))
				AutoConsume:Clean(tenacityEvents.AttributeChanged.Event:Connect(function(attribute)
...
run(function()
	...
	local function CreateWindow(self)
		...
		local function accent() return tenacity:GetThemeColor(0.12) end

		local window = Instance.new('Frame')
		window.Name = 'TenacityHotbarEditor'
		...
		window.ZIndex = 220
		window.Parent = tenacity.gui.ScaledGui
		...
		local title = Instance.new('TextLabel'); title.BackgroundTransparency = 1; title.Position = UDim2.fromOffset(18,0); title.Size = UDim2.new(1,-100,1,0); title.Text = 'Tenacity  /  Hotbar layout'; title.TextColor3 = text; title.TextSize = 15; title.FontFace = uipallet.FontSemiBold or uipallet.Font; title.TextXAlignment = Enum.TextXAlignment.Left; title.ZIndex = 222; title.Parent = header
		...
		close.MouseButton1Click:Connect(function() window.Visible=false; tenacity.gui.ScaledGui.ClickGui.Visible=true end)
		...
		function self:OpenEditor() refreshSlots();tenacity.gui.ScaledGui.ClickGui.Visible=false;window.Visible=true end
	...
	tenacity.Components.HotbarList=function(optionsettings,children,api)
		...
		local function selectHotbar(index)
			...
			for i,data in optionapi.Hotbars do if data.Object then data.Object.BackgroundColor3=i==optionapi.Selected and tenacity:GetThemeColor(.12):Lerp(surface,.78)or raised;data.Edge.Color=i==optionapi.Selected and tenacity:GetThemeColor(.12)or Color3.new(1,1,1);data.Edge.Transparency=i==optionapi.Selected and .55 or .94 end end
		...
		function optionapi:AddHotbar(data)
			...
			local dot=Instance.new('Frame');dot.BackgroundColor3=tenacity:GetThemeColor(index*.04);dot.BorderSizePixel=0;dot.Position=UDim2.fromOffset(8,17);dot.Size=UDim2.fromOffset(7,7);dot.Parent=row;roundGui(dot,4)
	...
	AutoHotbar = tenacity:Module('Player', {
		...
		Function = function(callback)
			if callback then
				...
				AutoHotbar:Clean(tenacityEvents.InventoryAmountChanged.Event:Connect(sortCallback))
			end
		end,
...
run(function()
	...
	BedBreakEffect = tenacity:Module('Vision', {
		Name = 'Bed Break Effect',
		Function = function(callback)
			if callback then
	            BedBreakEffect:Clean(tenacityEvents.BedwarsBedBreak.Event:Connect(function(data)
...
run(function()
	...
	local Crosshair = tenacity:Module('Vision', {
		Name = 'Crosshair',
		Function = function(callback)
			if callback then
...
run(function()
	...
	DamageIndicator = tenacity:Module('Vision', {
...
run(function()
	...
	FPSBoost = tenacity:Module('Vision', {
		Name = 'FPS Boost',
		Function = function(callback)
			if callback then
...
run(function()
	...
	HitColor = tenacity:Module('Vision', {
		Name = 'Hit Color',
		Function = function(callback)
			if callback then 
				repeat
...
run(function()
	...
	Interface = tenacity:Module('Vision', {
		Name = 'Interface',
		Function = function(callback)
...
run(function()
	...
	KillEffect = tenacity:Module('Vision', {
		Name = 'Kill Effect',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ReachDisplay = tenacity:Module('Vision', {
		Name = 'Reach Display',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SongBeats = tenacity:Module('Vision', {
		Name = 'Song Beats',
		Function = function(callback)
			if callback then
...
run(function()
	...
	SoundChanger = tenacity:Module('Vision', {
		Name = 'SoundChanger',
		Function = function(callback)
			if callback then
...
run(function()
	...
	UICleanup = tenacity:Module('Vision', {
		Name = 'UI Cleanup',
		Function = function(callback)
			for i, v in (callback and new or old) do
				for i2, v2 in v do
...
run(function()
	...
	Viewmodel = tenacity:Module('Vision', {
		Name = 'Viewmodel',
		Function = function(callback)
...
run(function()
	...
	WinEffect = tenacity:Module('Vision', {
		Name = 'WinEffect',
		Function = function(callback)
			if callback then
				WinEffect:Clean(tenacityEvents.MatchEndEvent.Event:Connect(function()
...
run(function()
	...
	BetterSpeed = tenacity:Module('Motion', {
		Name = 'Speed +',
		Function = function(callback)
...
run(function()
	...
	BetterFly = tenacity:Module('Motion', {
		Name = 'Fly +',
		Function = function(callback)
...
run(function()
	...
	TexturePacks = tenacity:Module('Vision', {
		Name = 'TexturePacks',
...
run(function()
	...
	InfiniteJump = tenacity:Module('Utility', {
		Name = 'InfiniteJump',
		Function = function(callback)
			if callback then
