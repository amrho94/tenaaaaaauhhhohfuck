return function(core)
    assert(type(core) == 'table' and core.Name == 'Tenacity', 'Tenacity API requires the Tenacity core')
    ...
    local function category(name)
        ...
        assert(result, ('Unknown Tenacity category %q'):format(tostring(name)))
        return result
    end

    local function normalizeSetting(spec)
        assert(type(spec) == 'table', 'Tenacity setting specification must be a table')
    ...
    local function addSetting(module, spec)
        ...
        assert(creator and type(module[creator]) == 'function', ('Unsupported Tenacity setting type %q'):format(kind))
        return module[creator](module, normalized)
    end

    local function decorate(module)
        if rawget(module, '__tenacity_api_v3') then return module end
        if rawget(module, 'Id') == nil then rawset(module, 'Id', module.Name) end
        rawset(module, '__tenacity_api_v3', true)
    ...
    function api.Modules:Register(categoryId, spec)
        assert(type(spec) == 'table' and type(spec.Name) == 'string', 'Tenacity module registration requires Name')
    ...
    function api:Notify(title, message, duration, kind)
        return core:CreateNotification(title or 'Tenacity', message, duration, kind)
    end
