...
do
	...
	if shared.TenacityRuntime then return shared.TenacityRuntime.Read(path, callback) end
		if not isfile(path) then
			...
			local success, data = pcall(function()
				return game:HttpGet('https://raw.githubusercontent.com/amrho94/tenaaaaaauhhhohfuck/'..readfile('tenacity/profiles/commit.txt')..'/'..select(1, path:gsub('tenacity/', '')), true)
			end)
...
for name, theme in {
		...
		['Tenacity Red'] = {{Color3.fromRGB(210, 39, 48), Color3.fromRGB(184, 25, 42)}, 9},
...
local themeNames = {'Tenacity', 'Spearmint', 'Jade Green', 'Green Spirit', 'Rosy Pink', 'Magenta', 'Hot Pink', 'Lavender', 'Amethyst', 'Purple Fire', 'Sunset Pink', 'Blaze Orange', 'Pink Blood', 'Pastel', 'Tenacity Red', 'Red Coffee', 'Deep Ocean', 'Chambray Blue', 'Mint Blue', 'Pacific Blue', 'Tropical Ice'}
...
function tenacity:LoadGUI()
	...
	for _, root in {
		...
		if root then
			local old = root:FindFirstChild('Tenacity')
	...
	tenacity.GUIStyle = guipane:CreateDropdown({
		...
		Function = function(value)
			...
			if tenacity.Interface then tenacity.Interface:SetMode(tenacity.GUIStyleName) end
	...
	tenacity:Clean(inputService.InputBegan:Connect(function(input)
		if tenacity.Interface and clickgui.Visible then
			local control=inputService:IsKeyDown(Enum.KeyCode.LeftControl) or inputService:IsKeyDown(Enum.KeyCode.RightControl)
			if tenacity.Interface.Binding or input.KeyCode==Enum.KeyCode.Escape or input.KeyCode==Enum.KeyCode.Slash
...
function tenacity:Uninject()
	...
	if shared.Tenacity == self then shared.Tenacity = nil end
	if shared.TenacityAPI and shared.TenacityAPI.Core == self then shared.TenacityAPI = nil end
	...
	Module = function(props, children, api)
		...
		function component:Toggle(multiple, synchronous)
			...
			if synchronous then
				...
				if not ok then warn('[Tenacity] '..props.Name..' cleanup failed: '..tostring(err)) end
			else
				task.spawn(props.Function, self.Enabled)
			end
		end
...
tenacity.ControlRegistry = setmetatable({}, {__mode = 'k'})
		...
		rawset(components, kind, function(props, parent, owner)
			...
			if type(component) == 'table' and props then
				...
				if registryOwner and (component.Type == 'Button' or (ownerOptions and ownerOptions[props.Name] == component)) then
					local controls = tenacity.ControlRegistry[registryOwner] or {}
					tenacity.ControlRegistry[registryOwner] = controls
...
run(function()
	...
	function ui:DrawControls(parent, owner, includeBind, style)
		...
		local controls, seen = {}, {}
		for _, option in tenacity.ControlRegistry[owner] or {} do
